...
export function useStatistics(initialPeriod: TimePeriod = "week") {
  ...
  const fetchAIRecommendations = useCallback(async () => {
    ...
    try {
      console.log(
        ...
        api.defaults.baseURL + "/recommendations",
      );
      ...
      if (response.data.success && Array.isArray(response.data.data)) {
        ...
        console.log("✅ Mapped recommendations:", recommendations.length);
      ...
      try {
        ...
        if (dailyGoals) {
          ...
          if (
            ...
            questionnaireResponse.data.data?.meals_per_day
          ) {
            const cleanedMeals = questionnaireResponse.data.data.meals_per_day
              .toString()
              .replace(/[^0-9]/g, "");
        ...
        const errorMessage =
          err.response?.data?.message || "Failed to load statistics data";
        setState((prev) => ({
          ...prev,
          isLoading: false,
          error: errorMessage,
      ...
      const baseData = [
        {
          ...
          description:
            language === "he" ? "צריכת קלוריות יומית" : "Daily calorie intake",
        ...
        },
        {
          ...
          category: "macros" as const,
          description:
            language === "he" ? "חלבון לבניית שרירים" : "Protein for muscles",
          trend: calculateTrend(
        ...
        },
        {
          ...
          category: "macros" as const,
          description:
            language === "he" ? "פחמימות לאנרגיה" : "Carbs for energy",
          trend: calculateTrend(
  ...
  const progressStats = useMemo((): ProgressStats => {
    ...
    return {
      ...
      averageCompletion: Math.round(
        state.statisticsData.averageCompletion || 0,
      ),
  ...
  const gamificationStats = useMemo((): GamificationStats => {
    ...
    const level =
      serverLevel && serverLevel > 0 ? serverLevel : calculatedLevel;
    ...
    const displayCurrentXP =
      state.statisticsData.currentXP !== undefined
